...
Page({
  ...
  //   var that = this;
  //   var db = wx.cloud.database();
  //   db.collection("member").orderBy('telephone', 'asc').get({
  //     success(res) {
  //       that.setData({
  //         member: res.data
  //       })
  ...
  login: function() {
    ...
    if (!myreg.test(phone)) {
      ...
      //     showCancel: false,
      //   })
    } else if (that.data.password.length != 0 && myreg.test(phone)) {
  ...
  //   })
  // for (var i = 0; i < register.length; i++) {
  //   if (register[i]._openid == this.data.openid) {
  //     return true;
  //   }
  // }
  // return false;
